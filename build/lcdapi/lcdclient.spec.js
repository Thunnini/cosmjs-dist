...
describe("LcdClient", () => {
    ...
    describe("withModules", () => {
        ...
        it("works for one extension", async () => {
            ...
            const client = lcdclient_1.LcdClient.withExtensions({ apiUrl: testutils_spec_1.wasmd.endpoint }, setupSupplyExtension);
        ...
        it("works for two extensions", async () => {
            testutils_spec_1.pendingWithoutWasmd();
            const client = lcdclient_1.LcdClient.withExtensions({ apiUrl: testutils_spec_1.wasmd.endpoint }, setupSupplyExtension, setupBankExtension);
            const supply = await client.supply.totalAll();
            expect(supply).toEqual({
                height: jasmine.stringMatching(testutils_spec_1.nonNegativeIntegerMatcher),
                result: [
    ...
    describe("broadcastTx", () => {
        ...
        it("can't send transaction with additional signatures", async () => {
            testutils_spec_1.pendingWithoutWasmd();
            const account1 = await secp256k1wallet_1.Secp256k1Wallet.fromMnemonic(testutils_spec_1.faucet.mnemonic, wallet_1.makeCosmoshubPath(0));
            const account2 = await secp256k1wallet_1.Secp256k1Wallet.fromMnemonic(testutils_spec_1.faucet.mnemonic, wallet_1.makeCosmoshubPath(1));
            const account3 = await secp256k1wallet_1.Secp256k1Wallet.fromMnemonic(testutils_spec_1.faucet.mnemonic, wallet_1.makeCosmoshubPath(2));
        ...
        it("can send multiple messages with one signature", async () => {
            testutils_spec_1.pendingWithoutWasmd();
            const wallet = await secp256k1wallet_1.Secp256k1Wallet.fromMnemonic(testutils_spec_1.faucet.mnemonic, wallet_1.makeCosmoshubPath(0));
            const accounts = await wallet.getAccounts();
            const [{ address: walletAddress }] = accounts;
            const memo = "My first contract on chain";
            const msg1 = {
